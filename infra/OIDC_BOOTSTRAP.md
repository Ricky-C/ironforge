...
**ADR-011 portal-lambda migration coverage** (verified 2026-05-04 during PR-A). ADR-011 introduces four new resources via the `portal-lambda` module: ECR repo `ironforge-portal`, Lambda function `ironforge-portal` (created in PR-B), IAM execution role `ironforge-portal-execution`, and log group `/aws/lambda/ironforge-portal`. All four fit existing prefix patterns: `WriteIronforgeECR` (`repository/ironforge-*`), `WriteIronforgeLambda` (`function:ironforge-*`, subsumes `lambda:UpdateFunctionCode` for CI deploy + `lambda:CreateFunctionUrlConfig` for the Function URL), `WriteIronforgeIAM` (`role/ironforge-*`), `WriteIronforgeLogs` (`/aws/lambda/ironforge-*`). No `aws iam put-role-policy` step required for that migration. Forward-readability: future migrations following the `ironforge-<component>` naming convention are similarly covered without explicit OIDC role expansion — verify naming match against these patterns before assuming a new sid is needed (per `feedback_iam_prefix_patterns_cover_forward.md`).
