...
These constitute the migration work, split across three PRs for rollback safety (subphase 2.2 minimal resumes after PR-B's verification gate passes):

- **PR-A: substrate preparation** — `portal-lambda` module creating ECR repo + IAM execution role + CloudWatch log group only (no Lambda function or Function URL yet); `OIDC_BOOTSTRAP.md` forward-readability note (existing prefix patterns already cover the new resources). No CloudFront changes; no production impact.
- **PR-B: cutover** — `apps/web/Dockerfile` + `next.config.mjs` swap + Lambda function (image-mode) + Function URL (`AUTH_AWS_IAM`) consuming PR-A's ECR + image-build script + `app-deploy.yml` rewrite + `cloudfront-frontend` origin switch (S3 → Function URL with OAC `origin_type = "lambda"`). Five-commit structure with the first-load-latency verification gate (task 7 below) between Lambda creation and CloudFront switch.
- **PR-C: legacy cleanup** — destroy the existing `ironforge-portal-<account-id>` S3 bucket after PR-B's verification gate passes plus a 24-48 hour stability window of Lambda serving without regressions.

The numbered tasks below describe the migration's content; the PR allocation is in the bullets above.
...
6. **CI OIDC apply role: no expansion needed.** Verified during PR-A (2026-05-04) that existing prefix-based patterns in `infra/OIDC_BOOTSTRAP.md` already cover the new portal substrate: `WriteIronforgeECR` (`ecr:*` on `repository/ironforge-*`) covers the `ironforge-portal` ECR repo; `WriteIronforgeLambda` (`lambda:*` on `function:ironforge-*`) subsumes `lambda:UpdateFunctionCode` and the `aws_lambda_function_url` resource's `lambda:CreateFunctionUrlConfig` etc.; `WriteIronforgeIAM` covers the execution role; `WriteIronforgeLogs` covers the log group. PR-A appends a forward-readability note to `OIDC_BOOTSTRAP.md` documenting the coverage. No `aws iam put-role-policy` step required for the migration; the lesson is captured in `feedback_iam_prefix_patterns_cover_forward.md`.
7. **Verify first-load latency meets UX expectations.** Runs as the gate between PR-B's Lambda-creation commit (commit 4 of the five-commit structure) and the CloudFront origin-switch commit (commit 5). The signed-curl measurement (`awscurl` against the Lambda Function URL with the `AUTH_AWS_IAM`-required SigV4 signature, 5 cold-start measurements with 10-minute idle each, take median) confirms the Lambda + Function URL substrate works end-to-end before traffic flips. If median > 3 seconds OR the curl returns non-200 / non-Next.js HTML, halt PR-B at commit 4 (Lambda exists but CloudFront still points at S3 — rollback is a no-op) and apply the documented mitigation order (hybrid static-asset split first; App Runner or provisioned concurrency only if the cost case is closed). PR-C's S3 destroy is additionally gated by 24-48 hours of stable post-cutover serving without regressions.
...
### 2026-05-05 (PR-C) — 24-48h post-cutover stability window skipped at portfolio scale; cold-start gate + functional checks remain the active PR-C precondition

**What was changed.** The "24-48 hour stability window of Lambda serving without regressions" gate (described at lines 151 and 161 above) was skipped for PR-C 2026-05-05. PR-C proceeded immediately after the cold-start verification gate (median 0.094s, well under the 2s threshold) and the post-recovery functional checks (portal `/` HTTP 200 with full Phase 0 portfolio page, `/api/health` HTTP 200, Lambda image `03d630c2...` healthy, CloudFront 5xx rate 0.0 across 24h).

**Why.** The soak gate's premise is *real user traffic exposing edge cases that synthetic verification doesn't catch*. At portfolio scale — single tenant, zero real users, zero baseline traffic — that premise doesn't apply. The 24-48h wait is ceremony, not risk mitigation: no real traffic during the window means the soak adds no information beyond what the cold-start gate + functional checks already produced. The verification work that DID matter (cold-start gate, end-to-end Lambda chain, CloudFront → OAC → Function URL → Lambda → LWA → Next.js wiring, all three recovery PRs #107/#108/#109) had already passed.

**What this preserves.** The cold-start verification gate (ADR-011 § "When to reconsider", task 7 above) remains the active PR-C precondition — its "median > 3 seconds OR non-200 / non-Next.js HTML → halt" semantics are unchanged. PR-B's rollback affordance (S3 bucket + s3-portal origin retained through PR-B) was preserved through the migration; PR-C is the deliberate point where that affordance closes (acknowledged in `docs/runbook.md § 13 § Rollback prerequisites § Failure mode 1`). The soak gate as ORIGINALLY DESIGNED remains the right shape for a production deployment with real user traffic — this amendment captures the portfolio-scale deviation, not a general policy change.

**Lesson.** Verification gates inherit their value from the failure modes they catch. A gate designed for *real-user edge cases* has no value at *zero-real-user scale*; running it anyway is process-for-process's-sake. Future gates should be evaluated against the failure mode they're meant to surface; if the failure mode can't manifest in the current deployment context, the gate is ceremony.

**When this amendment lapses.** If the platform reaches a deployment context with real user traffic (Phase 3+ public demo with actual visitors, multi-tenant operation, customer-facing SLA), the original soak gate's value reactivates and the next PR-C-shaped destroy of a rollback escape-hatch should run the full 24-48h window. The amendment is a portfolio-scale carve-out, not a permanent retirement.
