...
- **Cause:** Terraform's refresh phase issues read API calls (HeadBucket / DescribeX / GetX) against AWS to reconcile state. A transient API response — eventual-consistency delay, IAM permissions hiccup, throttle returning a non-success code — can be interpreted by terraform as "the resource no longer exists" and removed from state. The subsequent plan then proposes `+ create` for the supposedly-deleted resource and `must be replaced` for everything that depended on it (because their parent's id is now `(known after apply)`). At apply time, the destroys against AWS succeed (the dependent resources were real), but the parent recreate fails because the original parent never actually went away.
- **How to diagnose:** When a plan shows surprising `+ create` or `must be replaced` cascades against a resource that was untouched in the diff, **stop and verify the resource exists in AWS via the CLI before approving Apply**. For S3: `aws s3api head-bucket --bucket <name>`. For IAM policies: `aws iam get-policy --policy-arn <arn>`. For KMS keys: `aws kms describe-key --key-id <id>`. If AWS confirms the resource exists, the refresh's "deleted" claim was wrong and the plan is unsafe to apply — proceeding will destroy the dependents (which is recoverable) and then fail (leaving partial state, which is the real damage).
...
- **Case study:** PR #35's apply log: `# module.artifacts.aws_s3_bucket.artifacts has been deleted`. Refresh got a false positive on the artifacts S3 bucket; plan proposed creating it fresh and replacing the 5 dependent sub-resources. Apply destroyed the 5 sub-resources (versioning, encryption, public-access-block, lifecycle, bucket policy) successfully, then failed at bucket recreate with `BucketAlreadyExists`. PR #36 hit the same wall. PR #37 recovered via an `import` block (`infra/envs/shared/imports.tf`); the same apply re-created the 5 sub-resources, restoring full security posture. Bucket was empty (Phase 0 has no consumers), so no data risk; if it had held data, the destroyed bucket-policy + public-access-block window would have been a real exposure.
- **Prevention:** Treat refresh "deleted" claims as suspicious by default — verify with the AWS CLI before approving Apply on any plan that shows unexpected `+ create` or `must be replaced` for resources not modified in the diff. The CI plan/apply gate (`environment: production` with required reviewer + 5-minute wait) is the right place to catch these; the wait window is exactly long enough to run a `head-bucket`/`get-policy`/`describe-key` cross-check on any surprise in the plan.
- **Where:** This breadcrumb. The recovery pattern is reusable for any Terraform-managed AWS resource hit by refresh false-positives, not just S3 buckets.
