...
#### Portal health: add an end-to-end Synthetics canary on the real URL (metric alarms shipped)

- **What — RESOLVED (detection gap) / OPEN (richer signal):** The portal returned CloudFront 502 (`BadGatewayException`) for ~1 month (≈2026-05-27 → 2026-06-29) with **zero alerting** — the daily keepalive's async invokes were dropped silently (`AWS/Lambda AsyncEventsDropped`) without moving `FailedInvocations`/`Invocations`/`Errors`, so every health metric read green while the site was down. **Two CloudWatch alarms now close that gap** (`infra/envs/shared/portal-monitoring.tf`): `ironforge-portal-keepalive-dropped` (`AsyncEventsDropped >= 1` — the exact observed signature) and `ironforge-portal-not-invoked-24h` (`Invocations < 1` over 24h, missing-data=breaching — catches the keepalive rule itself going away), both → the CMK-encrypted `ironforge-ops-alerts` SNS topic → email. What's **still open** is an end-to-end *synthetic* check of the real user URL: the metric alarms watch the Lambda, not the full path, so they wouldn't catch a CloudFront/cert/DNS regression or the function 500ing on real routes while the keepalive's `/api/health` path still works.
- **Why deferred:** The metric alarms close the silent-outage gap that caused the incident (the highest-value half) for ~$1/mo (the CMK). A Synthetics canary adds genuine end-to-end coverage but also a canary Lambda, an S3 results bucket + lifecycle, an IAM role, a script, and per-run cost — justified as part of an observability arc, not bundled into the incident fix.
- **When to revisit:** Observability arc, or before relying on the portal for a high-stakes live demo / interview walkthrough.
- **Action:** Add a CloudWatch Synthetics canary doing `GET https://ironforge.rickycaballero.com/api/health` (and optionally `/`) every N minutes; alarm on `SuccessPercent < 100` (or `Failed > 0`) → reuse `aws_sns_topic.ops_alerts`. Optionally add an on-failure destination / DLQ to the keepalive's async-invoke config so dropped pings are captured for forensics rather than lost.
- **Where:** `infra/envs/shared/portal-monitoring.tf` (alarms + `ops_alerts` topic already here — add the canary alongside). Reference: `docs/runbook.md` § 5; ADR-003 § "Amendment — 2026-06-29" (the CMK justification).
...
#### Portal Lambda concurrency limit pending traffic data

- **What:** `aws_lambda_function.portal` in `infra/envs/shared/main.tf` does not set `reserved_concurrent_executions`. Checkov `CKV_AWS_115` flags this as a missing function-level concurrency cap. The `cost_reporter` Lambda gained `reserved_concurrent_executions=1` in PR-B (daily cron, no concurrency benefit); the portal cannot adopt a fixed value without traffic data.
- **Why deferred:** The right value depends on portal traffic patterns. Too low triggers `429 TooManyRequests` for legitimate users when concurrent requests exceed the cap — visible as cold-path failures during high-traffic moments (interview demos, social shares). Too high allows cost blowout under abuse (a small bot army could rack up Lambda invocation cost). Picking a value before there's real traffic data would be guesswork on either side.
- **When to revisit:** Once the portal has at least 4 weeks of production traffic captured in CloudWatch Lambda metrics. Practically: review the `ConcurrentExecutions` max + p99 + p50 over a 4-week window; set `reserved_concurrent_executions` to ~3× the observed p99 to absorb spikes while still capping abuse-driven cost. Re-evaluate quarterly or whenever traffic shape changes meaningfully (post-launch announcement, new feature driving traffic, etc.).
- **Action:** Add `reserved_concurrent_executions = <value>` to `aws_lambda_function.portal` based on observed traffic. Add a CloudWatch alarm on the `Throttles` metric crossing >0 (any throttle indicates the cap is too low, or a real abuse event — both need response). Remove `CKV_AWS_115` from `infra/.checkov.yml`'s skip-check once the portal fix lands.
- **Where:** `infra/envs/shared/main.tf` (`aws_lambda_function.portal` resource); `infra/.checkov.yml` (the skip-check entry references this debt entry).
...
#### GitHub App private key custom rotation Lambda

- **What:** `aws_secretsmanager_secret.github_app_private_key` in `infra/modules/github-app-secret/main.tf` has no automatic rotation configured. Checkov `CKV2_AWS_57` flags this. The secret holds the GitHub App's PEM private key; rotation requires regenerating the key in GitHub App settings (UI or `gh api`), then updating the Secrets Manager value. No off-the-shelf Secrets Manager rotation Lambda fits — the secret value originates outside AWS and the rotation protocol is GitHub-specific.
- **Why deferred:** Building a custom rotation Lambda is non-trivial. (a) The GitHub Apps API exposes `POST /apps/{client_id}/private-keys` to create a new key + return the new PEM. (b) The Lambda must call it as the app itself, requiring a bootstrap key separate from the rotated one (chicken-and-egg if the rotation Lambda mints its own keys). (c) The secret-versioning flow must coordinate so consuming Lambdas (create-repo, trigger-deploy) pick up the new key without a window of failed authentications — Secrets Manager's `AWSPENDING` → `AWSCURRENT` staging is the right primitive but needs careful wiring with `getInstallationToken`'s caching. (d) The old key needs deletion via `DELETE /apps/{client_id}/private-keys/{key_id}` after consumers transition. Manual rotation (regenerate in GitHub UI, `aws secretsmanager put-secret-value`) covers the operational need at current scope (single key, single operator, no compliance requirement forcing automation).
- **When to revisit:** Any of: (a) a compliance framework lands that requires automated key rotation with attestation (SOC 2 Type II, PCI DSS, certain enterprise procurement checklists); (b) the GitHub App is shared with collaborators making manual rotation coordination painful; (c) a key compromise occurs and post-mortem flags the manual-rotation delay as a contributing factor; (d) a published GitHub security advisory recommends rotation cadence shorter than the team's manual cadence sustains.
- **Action:** Build `services/maintenance/rotate-github-app-key/` Lambda. Bootstrap with a long-lived "master" key separate from the rotation target (stored in a separate Secrets Manager secret, manually rotated). The Lambda reads current secret version, calls GitHub `POST /apps/{client_id}/private-keys`, writes the new key as a new Secrets Manager version with stage `AWSPENDING`, validates it (`POST /app/installations/{id}/access_tokens` succeeds), promotes `AWSPENDING` → `AWSCURRENT`, invalidates the in-memory PEM cache in consuming Lambdas (publish to an EventBridge bus consumers subscribe to, or rely on the existing cache TTL), then calls `DELETE /apps/{client_id}/private-keys/{key_id}` for the old key. Schedule via Secrets Manager `automatically_after_days` or EventBridge cron (quarterly). Remove `CKV2_AWS_57` from `infra/.checkov.yml`'s skip-check once landed.
- **Where:** `infra/modules/github-app-secret/main.tf` (currently no rotation config); a new `services/maintenance/rotate-github-app-key/` Lambda would land; `packages/shared-utils/src/github-app/get-installation-token.ts` (PEM cache invalidation surface); `infra/.checkov.yml` (skip-check entry references this debt entry).

### Terraform / AWS provider

#### `force_destroy` on `aws_s3_bucket` must be in state at destroy time — two failure modes

- **What:** Terraform's S3 destroy uses the `DeleteBucket` API directly, which fails on non-empty buckets. Terraform's iterative-version-delete logic (the workaround that empties versioned buckets before calling `DeleteBucket`) only runs when `force_destroy = true` is in STATE at destroy time. Without that, terraform-driven bucket destruction leaves the bucket-empty step to the operator. Two distinct triggers share this root cause:
  1. **Late flip (Phase 1.5 — run-terraform deprovision).** `force_destroy = true` was added to a bucket config that had already been applied without it. `terraform destroy` reads `force_destroy` from STATE (where it remained `false`), not from the updated config — the destroy-time `Delete` handler in the AWS provider operates on `ResourceData` populated from state. To make a config-only attribute take effect, a `terraform apply` is required first to sync state with config; only after that can `terraform destroy` see the new value. The `run-terraform` Lambda's deprovision path doesn't run `apply` before `destroy`, so the late flip never propagated.
  2. **Resource removal (PR-C — portal-bucket destroy via apply).** `force_destroy` was always `false` from initial creation. The resource block was removed from terraform config in PR-C; CI's apply attempted `DeleteBucket` directly; failed with `BucketNotEmpty` (184 object versions + 32 delete markers from Phase 0 deploys). Different trigger from the late flip — there was no apply pass available because the resource block was being removed entirely — but the same failure surface.
- **Concrete impact (Phase 1.5 — late flip):** `templates/static-site/terraform/main.tf`'s `aws_s3_bucket.origin` was missing `force_destroy = true` until PR #82. Forward-only fix: services created post-#82 have `force_destroy=true` in state from day 1 and deprovision cleanly. Services created before #82 needed either (a) manual bucket-empty via AWS CLI before deprovision retry, or (b) state surgery to update the bucket attribute. Only `portfolio-demo` was in this state; we used (a) once. No existing pre-#82 services remain, so this trigger is dormant.
- **Concrete impact (PR-C — resource removal):** `infra/modules/cloudfront-frontend/main.tf`'s `aws_s3_bucket.portal` had `force_destroy = false` from creation. PR-C's apply hit `BucketNotEmpty`. Recovery: manual versions+markers cleanup via two `aws s3api delete-objects` calls (one for `Versions[]`, one for `DeleteMarkers[]`), then `gh run rerun --failed <id>`. Verified-working procedure captured in `docs/runbook.md` § 14.
- **Why deferred:** No active impact post-recovery. The class of bug only surfaces when terraform tries to destroy a non-empty bucket via state that doesn't authorize iterative emptying. Future surface is (i) any new bucket created with `force_destroy = false` whose resource block may eventually be removed, or (ii) any future late-flip scenario at multi-tenant scale. Both are addressable via the forward-discipline mitigation below.
- **When to revisit (any one of):**
  - **Adding another config-only destroy-time attribute** to a resource that's already been applied — e.g., `skip_final_snapshot=true` on an existing RDS instance, or `force_delete=true` on ECR repos that have images. Same late-flip pattern.
  - **Removing a resource block** for a bucket that has accumulated content (versioned or not). Pre-flight check before merging: `aws s3api list-object-versions --bucket <name> --query '{Versions: length(Versions || \`[]\`), DeleteMarkers: length(DeleteMarkers || \`[]\`)}'` — if either is non-zero, plan the `delete-objects` step before approving the apply.
  - **Phase 2 multi-tenancy** — operators can't be on the hook to manually empty buckets per tenant.
- **Action — implementation candidates (forward discipline first; targeted mitigations if specific failures recur):**
  1. **Forward discipline (recommended default):** create new tracked buckets with `force_destroy = true` from day 1 unless compliance / ObjectLock / a specific reason requires `false`. Runtime cost is zero (attribute only matters at destroy time); recovery savings on future destroys are real.
  2. **Manual emptying procedure** (always available): `docs/runbook.md` § 14 captures the verified two-step versioned-bucket cleanup. Use when destruction surfaces unexpectedly.
  3. **Two-phase destroy in `run-terraform`** (Phase 1.5-style late-flip mitigation, if it returns at multi-tenant scale): before `terraform destroy`, run `terraform apply -refresh-only` and then `terraform apply -target=aws_s3_bucket.origin -auto-approve` to sync state with config for any config-only attributes. Trade-off: extra ~15-30s per destroy + risk of unintended apply-time changes if config drifted from intent. Mitigated by `-target` scoping.
  4. **AWS-SDK pre-destroy emptying** in `run-terraform` or in the destroy-chain package: detect bucket presence + non-empty, run `aws s3 rm --recursive` + delete-marker cleanup via SDK. Bypasses terraform's `force_destroy` entirely. Trade-off: re-implements a feature terraform already provides; harder to keep aligned with future bucket configs (e.g., ObjectLock).
- **Where:** `services/workflow/run-terraform/src/handle-event.ts` (destroy invocation site, line ~499); `templates/static-site/terraform/main.tf` (origin bucket — already `force_destroy=true` post-#82); `infra/modules/cloudfront-frontend/main.tf` (portal bucket — destroyed in PR-C); `docs/runbook.md` § 14 (recovery procedure); discovery contexts: `project_phase1_5_verification_complete.md` § "Run 3" (late flip); PR-C apply 2026-05-06 (resource removal).
...
#### Deprovision-failed handler assumes Job is in `running` state during failure transition

- **What:** The `deprovision-failed` Lambda handler attempts a `running → failed` transition on the Job entity when the deprovisioning workflow's terminal failure path runs. The transition uses a conditional update that expects `Job.status === "running"`. **However**, the `deprovision-terraform` step (the first deprovisioning workflow Lambda — i.e., run-terraform with `action="destroy"`) doesn't transition the Job from `queued → running` on entry. Side effect: when the API kickoff path creates the Job (status=`queued`) and the workflow's first step (`deprovision-terraform`) fails before any other Lambda transitions Job state, the deprovision-failed handler hits its conditional update against `queued`, the condition fails, and the handler errors with `IronforgeDeprovisionFailedHandlerError: Job ... in unexpected state during deprovision-failure transition`. SFN then ends in `FAILED` state with that handler's error as the cause.
- **Why latent:** the handler runs only when deprovisioning fails. Most deprovision flows succeed (single-template; terraform destroy is reliable when IAM is correct). The brittleness only surfaces when a deprovisioning step fails AND `Job.status` hasn't transitioned past `queued`. PR-H/PR-I's IAM gap was the specific failure path that hit this.
- **Empirical confirmation (PR-I post-IAM-fix retry, 2026-05-06):** Path-B SFN-direct retry of pr-h-smoke deprovision. run-terraform Lambda hit AccessDenied on `s3:ListBucketVersions` (the boundary gap PR-I addresses); deprovision-failed handler then ran and errored with the unexpected-state message. SFN ExecutionFailed; cause: `Deprovisioning workflow ended via DeprovisionFailed path. See JobStep entries and the $.error ResultPath for the originating failure.`
- **Why this is downstream noise (not in PR-I scope):** once the boundary fix lands and IAM works, the deprovision-terraform step succeeds; deprovision-failed handler doesn't run; the brittleness doesn't surface. The handler's expectation of `running` would still be wrong on the next failure path that fires before any state transition, but that requires a *new* failure mode in deprovision-terraform itself. PR-I unblocks the immediate path without touching this.
- **Related tech-debt:** `JobStep#deprovision-terraform observability gap` (entry above). Same root cause: deprovision-terraform doesn't transition Job state by design (the `action === "apply"` gate inside run-terraform prevents both the JobStep write AND the Job state transition for destroy invocations). Resolution candidates here naturally compose with that entry's resolution.
- **Resolution candidates:**
  1. **deprovision-terraform transitions Job on entry** (mirror provisioning's first-Lambda behavior). Smallest scope; fixes both this entry's handler brittleness AND part of the JobStep observability gap. Trade-off: violates the existing `action === "apply"` gate which exists for cleanup-on-failure's specific reason; refactor would distinguish "destroy-during-cleanup" (no Job transitions) from "destroy-during-deprovision" (transitions expected).
  2. **deprovision-failed handler accepts both states** (`queued OR running`). Smaller surface; only handler changes. Trade-off: accepts a state shape that's "wrong" semantically (queued shouldn't reach the failed-handler in a healthy workflow).
  3. **Refactor deprovision lifecycle separately from provisioning** — give deprovision its own Lambda(s) that don't share run-terraform's apply-gated state-write logic. Largest scope; cleanest separation; pairs with the JobStep observability gap entry's "second action=destroy consumer" trigger.
- **When to revisit (any one of):**
  - **Deprovision failures become operationally meaningful.** Currently rare (single-tenant, single-operator); when multi-tenant or higher provisioning rates land, deprovision failures + their handler-state brittleness compound.
  - **Same brittleness fires on a different failure path** — e.g., a future deprovision step also fails before any Job transition (network partition, Lambda timeout, etc.). At that point the handler's conditional update being too narrow is repeated symptom; resolution forces.
  - **Resolving JobStep observability gap (the entry above) — both share the same `action === "apply"` gate; a refactor that touches one naturally addresses the other.**
- **Where:** `services/workflow/deprovision-failed/src/handle-event.ts` (the conditional update site); `services/workflow/run-terraform/src/handle-event.ts` (where the `action === "apply"` gate prevents the queued → running transition); the conditional update would need to either widen its expected state set or the upstream Lambda would need to write the transition first.
...
#### Portal local Docker build fails on workspace module resolution

- **What:** `docker build -f apps/web/Dockerfile -t ironforge-portal:scan .` (or equivalent) fails locally with `ERR_PNPM_RECURSIVE_RUN_FIRST_FAIL` during `pnpm --filter @ironforge/web build`. Next.js cannot resolve `@ironforge/shared-types/src/*.ts` files from inside the builder stage — the workspace symlink resolves but the source files behind it appear missing to Next's webpack pass. CI builds (via `docker/build-push-action@v5` with `platforms: linux/arm64`) succeed against the same Dockerfile.
- **Why deferred:** Discovered during PR-C's local-triage attempt for trivy image scanning. The CI scan path (Trivy against the live ECR image) gives full production coverage; the local-triage gap was worked around using base-image + filesystem scanning rather than chasing the workspace-resolution issue. Triage-time fix would have been a rabbit hole (probably a pnpm-strict-mode + Next-server-side-tracing interaction, or an arch/buildx subtlety; CI uses `linux/arm64` via buildx + QEMU on x86_64 runners) that's disconnected from PR-C's security-gate goal.
- **When to revisit:** Any of: (a) a developer needs to debug the portal image locally (e.g., reproducing a CI-only issue that doesn't surface in `pnpm dev`); (b) a future security workflow needs a local-build path as part of its triage (PR-C used a workaround that won't generalize to every tool); (c) the CI image build itself starts failing in a way that local reproduction would diagnose faster than CI iterations.
- **Action:** Reproduce the failure cleanly (capture exact `docker build` invocation + full output). Test hypotheses in order: (1) `docker buildx build --platform linux/arm64` (matches CI exactly via QEMU emulation) — if this works, the issue is x86_64-native build pathology; (2) add `transpilePackages: ["@ironforge/shared-types"]` to `apps/web/next.config.mjs` (likely fix for Next.js workspace-source-resolution); (3) verify pnpm's `node-linker` setting (CI may use a different default); (4) inspect the builder stage with `docker run --rm -it <staged-image> sh` to check what's actually in `/repo/packages/shared-types/src/` at build time. Document the fix in `docs/runbook.md` § local development.
- **Where:** `apps/web/Dockerfile` (builder stage that runs `pnpm --filter @ironforge/web build`); `apps/web/next.config.mjs` (likely fix location via `transpilePackages`); `docs/runbook.md` (local-dev section to update once resolved).
...
#### Portal (`apps/web`) vitest setup deferred — schema-level tests in shared-types cover envelope parsing

- **What:** `apps/web` has no vitest configuration or test runner setup. Other workspaces (`packages/*`, `services/*`) have vitest wired with co-located `*.test.ts` files (~541 tests at PR-E time). The portal has zero tests beyond TypeScript's typecheck and Next.js's build-time validation. PR-E (subphase 2.2 DELETE) deferred adding api-client integration tests on this basis: the load-bearing logic (envelope parsing for both 202 success and 409 error shapes) is covered by `DeprovisionServiceResponseSchema` tests in `packages/shared-types/src/service.test.ts` and the existing `ApiResponseSchema` envelope tests in `api.test.ts`. The api-client's `request()` helper is unchanged from PR-D; the new `deprovisionService` method is a one-line composition with the schema, identical pattern to `getService`.
- **Why deferred:** Setting up vitest in `apps/web` is its own scope (~1-2 hours): add `vitest`, `@vitejs/plugin-react`, `@testing-library/react`, `jsdom` (or `happy-dom`) as devDeps; write a `vitest.config.ts` with the right resolver (workspace package handling, JSX transform); add a `test` script; choose a co-located vs `__tests__/` convention. Adding all of that inside a feature PR conflates "wire the test infrastructure" with "implement the feature" in one review surface — same anti-pattern called out in the ESLint exhaustiveness entry above. Schema-level tests at `shared-types` cover where things break (envelope parsing, malformed shapes); the marginal value of api-client integration tests at portal-vitest level is small until either (a) component-level behavior tests are needed, or (b) multiple api-client methods accumulate that warrant their own test surface.
- **When to revisit (any one of):**
  - **Component-level behavior tests needed.** First time TypeScript + a build pass aren't enough — e.g., a complex hook with state machine, a component with conditional render branches that are easy to regress. Likely trigger: subphase 2.4 polling (state transitions in the detail page driven by Job status changes).
  - **Multiple api-client methods accumulate.** PR-D added `getService`; PR-E added `deprovisionService`. By 2.3 (wizard) there will likely be `createService`, `listServices`, possibly `getJob` for polling. At ~5 methods, dedicated api-client integration tests (mock fetch + assert thrown `ApiClientError` shapes) become worth the setup cost.
  - **Page-level tests for hooks + state interactions.** `app/services/[id]/page.tsx` already has a `DeprovisionAction` component with state interactions (`open`, mutation lifecycle, cache seeding). Today verified by typecheck + browser smoke; eventually warrants render-tree assertions.
  - **Demo mode (subphase 2.6) lands.** Demo mode reuses the same components but swaps the api-client target — exactly the seam where a tests would catch regressions cheaply.
...
#### AWS auto-injects `LambdaECRImageRetrievalPolicy` on image-mode Lambda image updates (recurring drift)

- **Symptom:** `terraform plan` against an `aws_ecr_repository_policy` resource that backs an image-mode Lambda shows an in-place change removing a statement with `Sid = "LambdaECRImageRetrievalPolicy"` granting `lambda.amazonaws.com` (service principal) `ecr:BatchGetImage`/`ecr:GetDownloadUrlForLayer`/`ecr:Set|Delete|GetRepositoryPolicy` with `aws:sourceArn` conditioned on `arn:aws:lambda:<region>:<account>:function:*`. The statement is not in our terraform; terraform wants to remove it because the resource manages the FULL policy, not a subset. **The drift recurs** after each Lambda image-update operation.
- **Cause — actual pattern:** Documented AWS behavior. Lambda ensures ECR pull permissions at cold-start regardless of operator config by auto-injecting this statement on `CreateFunction` AND on `UpdateFunctionCode` for image-mode functions. Not a one-shot at creation — it fires on every image-mode update.
- **Empirical evidence (PR-C apply 2026-05-06):**
  - 03:38 UTC — shared composition apply reconciled `module.terraform_lambda_image.aws_ecr_repository_policy.run_terraform` (removed `LambdaECRImageRetrievalPolicy`).
  - 03:40 UTC — dev composition apply ran `module.task_run_terraform.aws_lambda_function.this: Modifying...` (CI rebuilt + pushed image; terraform set new `image_uri`).
  - Post-apply: ECR policy has both statements again. AWS re-injected during the `UpdateFunctionCode`-equivalent operation.
  - Same apply: `ironforge-portal` ECR policy shows no drift because the portal Lambda function was not modified in this apply (PR-C touched only `cloudfront-frontend`, not `portal-lambda`). Portal would surface the same drift on its next image rebuild.
  - The original framing of this entry (added in PR-C's `chore(infra)` commit) characterized the injection as one-shot — empirically wrong; corrected here.
- **How to diagnose:** When a plan shows the statement-removal diff against an `aws_ecr_repository_policy` resource:
  - Verify the injected shape via `aws ecr get-repository-policy --repository-name <name>`. Injected statement has `Sid = "LambdaECRImageRetrievalPolicy"`, `Principal.Service = "lambda.amazonaws.com"`, the `aws:sourceArn` wildcard condition above.
  - If only `LambdaECRImageRetrievalPolicy` is present beyond terraform-managed statements, apply will reconcile cleanly. Drift will return on next Lambda image update for that repo.
  - If additional non-terraform statements (other Sids, other principals) are present, investigate before applying — could indicate console intervention or another tool managing the policy.
- **Resolution options (deferred to follow-up PR — out of scope for the docs PR that corrects this entry):**
  - **Option Y: Adopt the auto-injected statement in terraform.** Add `LambdaECRImageRetrievalPolicy` to the module's `data.aws_iam_policy_document.repository`; both statements become terraform-tracked; AWS injection becomes idempotent (no diff). Trade-off: managing a statement AWS would otherwise auto-inject. Slight surface expansion in exchange for "no recurring drift" legibility.
  - **Option Z: Investigate the config delta between `portal-lambda` and `terraform-lambda-image`.** Both modules define an `aws_ecr_repository_policy` with similar shape (single Sid, AWS principal, object-level pull actions); both are image-mode Lambdas in the same account/region; only `terraform-lambda-image` shows recurring drift in PR-C's apply. The delta — possibly cross-composition role reference (run-terraform's role lives in the dev composition; portal's role lives in shared) or some other asymmetry — may be what triggers the auto-injection. Identifying it might let us replicate the portal-lambda shape on `terraform-lambda-image` and eliminate the drift without adopting AWS's statement.
  - **Lean Z first.** Investigation cost ~30-60 min (compare modules side-by-side; check AWS docs on the auto-injection trigger conditions). Outcomes:
    - Z succeeds → drift eliminated without managing AWS's statement.
    - Z reveals Y is necessary (e.g., the asymmetry is intrinsic to cross-composition role references) → adopt Y on the established understanding.
    - Z reveals portal-lambda will start showing the same drift on its next image update → Y becomes the only path.
    - Z reveals a third option (provider-side toggle, AWS-side workaround) → take that.
  - Premature commitment to Y forecloses cleaner solutions discoverable cheaply.
- **DO NOT add `lifecycle.ignore_changes = [policy]`.** Masks future legitimate policy changes; strictly worse failure mode than the recurring drift.
- **Triggers for revisit:**
  - Drift loop becomes operationally annoying (every shared plan showing the same diff is a small per-instance cost; aggregate annoyance + plan-review fatigue accelerates).
  - Future image-mode Lambdas widen the affected surface (each new repo with an `aws_ecr_repository_policy` adds a drift point).
  - CI changes increase image-update frequency (e.g., scheduled rebuilds; multi-image deploys; portal-lambda image rebuilds becoming routine post-2.2).
- **Case study:** PR-C (2026-05-05/06). The chore commit bundling the reconciliation framed the injection as one-shot. PR-C apply failed for unrelated reasons (force_destroy on a versioned bucket; CloudFront OAC parallelization race). Recovery succeeded; post-recovery verification showed the ECR policy back to 2 statements; the one-shot framing was falsified. This corrected entry replaces the original; the convention added at `docs/conventions.md` § "ADR/tech-debt empirical claims require verification" is the upstream change that would have caught the wrong framing earlier.
- **Where:** `infra/modules/terraform-lambda-image/main.tf` (`aws_ecr_repository_policy.run_terraform` — recurring drift observed); `infra/modules/portal-lambda/main.tf` (`aws_ecr_repository_policy.portal` — no observed drift in PR-C apply, expected on next portal image rebuild); `docs/conventions.md` § "ADR/tech-debt empirical claims require verification" (the upstream convention).

#### Terraform parallelizes destroys with in-place modifications across the same apply

- **Symptom:** A plan shows in-place update on resource A AND destroy of resource B, where A currently references B but the desired-state config for A does not. Apply fails with an "in use" error on the destroy of B (CloudFront `OriginAccessControlInUse`, IAM `EntityRelationshipExists`, RDS `ResourceInUse`, etc.) while the in-place update on A's reference-removal never executes (apply log shows `Refreshing` for A but no `Modifying`).
- **Cause:** Terraform's planner builds the dependency graph from the **desired** state, not the **current** state. In the desired state, A and B are independent (A no longer references B; B no longer exists). The planner therefore parallelizes the in-place update on A with the destroy of B. The destroy of B races against the propagation of A's update; AWS's API rejects the destroy because A still references B at the moment the API call lands. Terraform's run aborts on the destroy error, often before A's in-place update has been issued at all (the destroy of B is short-circuited fast; A's update is long-running and gets cancelled). Net effect: zero progress on either change.
- **How to diagnose:** When apply fails with "in use" or equivalent on a destroyed resource AND the dependent resource's update was supposed to remove the reference in the same plan, check the apply log for the `Modifying` line on the dependent resource. If only `Refreshing` is present (no `Modifying`), the race fired. Verify live state via the AWS API (the dependent resource still references the destroyed one). Don't assume the dependent's update half-completed — it likely never started.
- **Mitigation patterns** (in order of preference for one-off recoveries; see `docs/runbook.md` § 15 for the verified-working PR-C procedure):
  - **Manual sequencing via AWS CLI** for the reference-removal phase, then `terraform apply` (or `gh run rerun --failed <id>`) for the destroy-only cleanup. After the manual update, terraform's refresh syncs state with reality; the next plan shows only the destroy left to do; apply proceeds without race. Used in PR-C recovery for the CloudFront distribution → S3 OAC case.
  - **Two-phase apply via separate PRs.** PR 1 removes the reference (in-place update only). PR 2 destroys the unreferenced resource. Forces serialization through the merge cycle. Right shape when the dependent resource's update is non-trivial or when scoping benefit justifies the overhead.
  - **Terraform `-target` phasing** for in-pinch cases (`terraform apply -target='<dependent>'` then full apply). Workflow-incompatible in our CI (apply-shared / apply-dev workflow doesn't expose `-target`); only useful when running terraform locally during recovery. Anti-pattern as a steady-state solution.
- **Triggers for awareness in plan review:**
  - Plan shows resource removal AND removal of a reference to that resource in the same diff.
  - Cross-module references where the planner can't see the ordering implied by current state.
  - Any case where current state has more dependencies than desired state.
  - Specifically with CloudFront: distribution origin block + OAC removal in the same plan is a known race shape.
- **Case study:** PR-C apply 2026-05-06. Plan output showed `module.portal_frontend.aws_cloudfront_distribution.portal will be updated in-place` (drop the s3-portal origin block) AND `aws_cloudfront_origin_access_control.portal will be destroyed`. Apply log: 03:31:43 plan output → 03:31:44 OAC destroy starts → 03:31:44 S3 bucket destroy starts → 03:31:44.67 S3 bucket destruction complete → ~1s later OAC destroy fails with `OriginAccessControlInUse`. The distribution `Modifying` line never appeared. Recovery: manually update the distribution config via `aws cloudfront update-distribution`, wait for `Deployed`, re-run apply (which then sees only the OAC destroy left). Captured in `docs/runbook.md` § 15.
- **Where:** Manifests as: `aws_cloudfront_distribution` ↔ `aws_cloudfront_origin_access_control`; can manifest with `aws_iam_role` ↔ `aws_iam_role_policy_attachment` if both are removed in the same plan; any AWS resource pair where one in-place-references another being destroyed. `docs/runbook.md` § 15 holds the verified-working recovery.
