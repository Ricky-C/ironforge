...
- **Why deferred:** No Phase 1 consumer yet exists. The workflow Lambda role that mints GitHub App installation tokens lands in a future commit. Adding the key-policy statement before the role exists would require a placeholder principal (account root again) or a pre-created role that does nothing — both worse than waiting. The secret itself is created and stored correctly during the bootstrap procedure; it just has zero current decrypters by design.
- **When to revisit:** First commit that introduces a Lambda needing GitHub App installation tokens — almost certainly `services/workflow/create-repo/` or a sibling task Lambda. The tech-debt note in the module body identifies the exact uncomment site.
- **Action:** In the Lambda-introducing commit: (1) uncomment the `AllowWorkflowLambdaDecrypt` skeleton in `infra/modules/github-app-secret/main.tf` and remove its activation header; (2) add `workflow_lambda_role_arn` as a module input variable; (3) pass the consuming Lambda's role ARN through from `infra/envs/shared/main.tf`; (4) verify the `kms:EncryptionContext:SecretARN` value is `StringEquals` against the imported secret ARN — exact match, not a wildcard. Pair with the consuming Lambda's identity policy: `secretsmanager:GetSecretValue` scoped to the exact secret ARN and `kms:Decrypt` scoped to the github-app CMK ARN with the same EncryptionContext condition.
- **Where:** `infra/modules/github-app-secret/main.tf` (the commented-out `statement` block in `data.aws_iam_policy_document.kms_key`); the consuming Lambda's identity-policy file when that module lands.
