...
#### `Job.currentStep` is a design-implementation mismatch — drop the field during 2.4-A.2

- **What:** `Job.currentStep` exists in `JobRunningSchema` (`packages/shared-types/src/job.ts`) and gets initialized to the first step name (e.g., `validate-inputs`) at job creation. Workflow Lambdas (validate-inputs, create-repo, generate-code, run-terraform, etc.) **do not update `Job.currentStep`** as they enter their stage. The field stays at its initial value for the entire workflow run, regardless of how far the SFN execution has actually progressed.
- **Why this isn't a bug:** the field exists in the schema (which suggests denormalization — a write-side maintained "current step" cache for read-efficiency). Workflow Lambdas don't maintain it (which suggests computation — read currentStep from the JobStep entries on demand). Two coherent designs; the current implementation is neither. **Design-implementation mismatch, not a behavior bug.**
- **Empirical confirmation (PR-G smoke, 2026-05-06):** PR-G's manual smoke provisioned `pr-h-smoke` (Job `43628d09-...`) and polled `Job.currentStep` directly via DynamoDB at 30-second intervals during a ~10m provisioning run. SFN advanced through 8 stages (validate-inputs → create-repo → generate-code → run-terraform → wait-for-cloudfront → trigger-deploy → wait-for-deploy → finalize). `Job.currentStep` stayed at `validate-inputs` the entire time. Final state at SUCCEEDED: `Job.currentStep` = `validate-inputs`. JobStep entries (PK=`JOB#<id>`, SK=`STEP#<name>`) are reliable — one per stage with accurate timestamps; the per-step truth lives there.
- **Two coherent resolutions:**
  - **Design 1 — maintain on write:** workflow Lambdas update `Job.currentStep` as they enter their stage (~1 line per Lambda). Read efficiency via denormalization. Cost: per-Lambda write surface; slight contention risk on hot paths.
  - **Design 2 — compute on read:** drop `Job.currentStep` from the schema entirely. When a consumer needs "where is this job," compute from `JobStep[]` (find the most recently-`startedAt` step that hasn't `completedAt`/`failedAt`). Write simplicity; one source of truth (JobStep entries).
- **Resolution chosen — Design 2:** drop `currentStep` from `JobRunningSchema` (and `JobBaseSchema` if it's there too) during subphase 2.4-A.2. 2.4-A.2 polls `JobStep[]` anyway (per ADR-010 + post-PR-G smoke design); `Job.currentStep` adds nothing once `JobStep[]` is the polling shape. Removing the field eliminates write-side maintenance burden and makes the schema honest about where "current step" data lives.
- **Removal trigger:** **PR shipping subphase 2.4-A.2 with the `JobStep[]` polling endpoint.** When that PR lands, this entry gets removed. The PR diff drops the field from `JobRunningSchema`, removes the initial-value write at job creation, and updates any consumers that read it (currently: none in production code per PR-G audit; the field was always informational). If 2.4-A.2 is deferred indefinitely (>3 months), revisit this entry — long-deferred field removal becomes its own coordination concern as schema migrations gain dependents.
- **Future Design 1 reconsideration:** if a future read path needs "list jobs across services with their current step efficiently" (e.g., an admin dashboard listing many in-flight jobs), the JobStep[] computation per row may be too expensive. At that point, denormalization on write becomes warranted, and Design 1 reactivates. Currently no such read path exists; defer.
- **Where:** `packages/shared-types/src/job.ts` § `JobRunningSchema.currentStep`; `services/workflow/_stub-lib/src/stub-task.ts` (initial write-site at job creation); future `services/workflow/*/src/handle-event.ts` per-Lambda update sites for Design 1 (not implemented).
