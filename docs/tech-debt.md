...
#### AWS-auto-injected `LambdaECRImageRetrievalPolicy` statement on image-mode Lambda creation

- **Symptom:** `terraform plan` against an `aws_ecr_repository_policy` resource that backs an image-mode Lambda shows an in-place change removing a statement with `Sid = "LambdaECRImageRetrievalPolicy"` granting `lambda.amazonaws.com` (service principal) `ecr:BatchGetImage`/`ecr:GetDownloadUrlForLayer`/`ecr:Set|Delete|GetRepositoryPolicy` with `aws:sourceArn` conditioned on `arn:aws:lambda:<region>:<account>:function:*`. The statement is NOT in our terraform — terraform wants to remove it because the resource manages the FULL policy, not a subset.
- **Cause:** AWS Lambda's CreateFunction API (and possibly UpdateFunctionCode in some image-mode paths) injects this statement onto the source ECR repo's policy as a side effect of creating an image-mode function from that repo. Mechanism is one-shot per function creation, not per invocation — once apply removes the auto-injected statement, AWS does not re-inject unless a new function is created from the same repo. The behavior is intermittent: image-mode Lambda creation sometimes triggers it, sometimes doesn't, and the conditions under which AWS injects vs. doesn't are not fully understood. Empirical observation in this repo: `ironforge-run-terraform` ECR repo (auto-injected during PR-C.6's run-terraform Lambda creation, 2026-05-02) vs. `ironforge-portal` ECR repo (no auto-injection during PR-B's portal Lambda creation, 2026-05-04). Both are image-mode Lambdas in the same account/region; the difference in outcome is unexplained.
- **How to diagnose:** When a plan shows the statement-removal diff against an `aws_ecr_repository_policy` resource, verify it's truly auto-injected (not a recent live edit) via `aws ecr get-repository-policy --repository-name <name>` and inspect the statements. If only `LambdaECRImageRetrievalPolicy` (or its variants) is present beyond the terraform-managed statements, it's safe to reconcile by applying the plan — AWS doesn't re-inject post-removal in steady state. If additional non-terraform statements are present, investigate before applying (could indicate live console intervention or another tool managing the policy).
- **Case study:** PR-C (2026-05-05) terraform plan against the shared composition surfaced this drift on `module.terraform_lambda_image.aws_ecr_repository_policy.run_terraform`. Live policy had 2 statements (`AllowRunTerraformLambdaPull` + `LambdaECRImageRetrievalPolicy`); terraform managed only the first. The `ironforge-portal` ECR repo, also image-mode and created in the same overall migration arc, had only the 1 terraform-managed statement (`AllowPortalLambdaPull`). PR-C bundled the run-terraform reconciliation as incidental drift (per the discipline-purpose pushback — split-PR overhead exceeds operational benefit when no rollback/verification asymmetry exists).
- **Prevention:** None at the AWS-injection layer (the behavior is provider-side and not toggleable). At the terraform-config layer: do NOT add `lifecycle.ignore_changes = [policy]` to the `aws_ecr_repository_policy` resource — that masks future legitimate config changes too, which is a worse failure mode than the periodic drift-reconciliation. Reconcile as incidental drift when surfaced in a plan; the next plan post-apply will be clean.
- **Where:** `infra/modules/terraform-lambda-image/main.tf` (`aws_ecr_repository_policy.run_terraform`); `infra/modules/portal-lambda/main.tf` (`aws_ecr_repository_policy.portal` — same shape, no observed drift to date).
